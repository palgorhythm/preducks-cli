...
function createActionFiles(path, appName, storeConfig, reducerName) {
  ...
  fs.writeFileSync(
    actionTypesFile,
    format(
      interfacesImportText +
        actionInterfacesText +
        actionTypesEnumText +
        typeGuardText,
      {
        parser: 'typescript'
...
function createReducerFiles(path, appName, storeConfig, reducerName) {
  ...
  const numberOfActions = Object.keys(storeConfig.reducers[reducerName].actions)
    .length;
  ...
  currentReducerStoreSliceKeys.forEach(storeSlicePropertyName => {
    ...
    try {
      const replacedStr = storeSlicePropObj.initialValue
        .replace(
          /(\w+) ?(:)/g,
          (wholeMatch, groupOne, groupTwo) => `"${groupOne}"${groupTwo}`
        )
        .replace(/'/g, '"');
